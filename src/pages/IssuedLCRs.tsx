...
import type { IssuedLCR } from '../types/lcr';

export function IssuedLCRs() {
  const [isFormVisible, setIsFormVisible] = useState(false);
  const [lcrs, setLCRs] = useState<IssuedLCR[]>([]);
  const [filteredLCRs, setFilteredLCRs] = useState<IssuedLCR[]>([]);
  ...
  const loadLCRs = async () => {
    try {
      const data = await api.issuedLCRs.list();
      setLCRs(data);
  ...
  const handleSubmit = async (data: any) => {
    setIsLoading(true);
    try {
      await api.issuedLCRs.create(data);
      await loadLCRs();
  ...
  const handleEdit = async (lcr: IssuedLCR) => {
    setIsLoading(true);
    try {
      await api.issuedLCRs.update(lcr.id, lcr);
      await loadLCRs();
    } catch (err) {
  ...
  const handleSearch = (filters: SearchFilters) => {
    ...
    if (filters.issuer) {
      filtered = filtered.filter(lcr =>
        lcr.beneficiaryName.toLowerCase().includes(filters.issuer.toLowerCase())
      );
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold text-gray-900">LCRs Émis</h1>
