...
import type { IssuedCheck } from '../types/check';

export function IssuedChecks() {
  const [isFormVisible, setIsFormVisible] = useState(false);
  const [checks, setChecks] = useState<IssuedCheck[]>([]);
  const [filteredChecks, setFilteredChecks] = useState<IssuedCheck[]>([]);
  ...
  const loadChecks = async () => {
    try {
      const data = await api.issuedChecks.list();
      setChecks(data);
  ...
  const handleSubmit = async (data: any) => {
    setIsLoading(true);
    try {
      await api.issuedChecks.create(data);
      await loadChecks();
  ...
  const handleEdit = async (check: IssuedCheck) => {
    setIsLoading(true);
    try {
      await api.issuedChecks.update(check.id, check);
      await loadChecks();
    } catch (err) {
  ...
  const handleSearch = (filters: SearchFilters) => {
    ...
    if (filters.issuer) {
      filtered = filtered.filter(check =>
        check.beneficiaryName.toLowerCase().includes(filters.issuer.toLowerCase())
      );
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold text-gray-900">Chèques Émis</h1>
