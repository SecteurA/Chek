...
import type { ReceivedCheck, IssuedCheck, ReceivedCheckStatus, IssuedCheckStatus } from '../../types/check';

const baseCheckSchema = {
  checkNumber: z.string().min(1, 'Numéro de chèque requis'),
...
const issuedCheckSchema = z.object({
  ...baseCheckSchema,
...
type IssuedCheckFormData = z.infer<typeof issuedCheckSchema>;

interface CheckFormProps {
  type: 'received' | 'issued';
  onSubmit: (data: ReceivedCheckFormData | IssuedCheckFormData) => Promise<void>;
  initialData?: Partial<ReceivedCheck | IssuedCheck>;
  isLoading?: boolean;
  onCancel?: () => void;
}

export function CheckForm({ type, onSubmit, initialData, isLoading, onCancel }: CheckFormProps) {
  const {
    register,
    handleSubmit,
    setValue,
    ...
    resolver: zodResolver(type === 'received' ? receivedCheckSchema : issuedCheckSchema),
    defaultValues: initialData
  });

  const currentStatus = watch('status');

  const statusOptions: (ReceivedCheckStatus | IssuedCheckStatus)[] = 
    type === 'received' 
